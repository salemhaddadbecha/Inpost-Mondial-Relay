...
def run_parcel_counts(events_df, apm_df, by_courier=False):
    ...
    weekly = compute_weekly_counts(stored, apm_df, by_courier)
    quarterly = compute_quarterly_counts(stored, apm_df, by_courier)
    avg_time = compute_avg_time_in_apm(events_df, apm_df)
