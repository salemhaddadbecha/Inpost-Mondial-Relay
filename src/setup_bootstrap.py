...
Path('data/batch_parquet').mkdir(parents=True, exist_ok=True)
Path('data/stream_input_parquet').mkdir(parents=True, exist_ok=True)
...
pq.write_table(tbl, 'data/batch_parquet/sample_fact_parcel_events.parquet')
pq.write_table(tbl, 'data/stream_input_parquet/sample_fact_parcel_events.parquet')
